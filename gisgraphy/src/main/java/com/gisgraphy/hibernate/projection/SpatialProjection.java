...
public class SpatialProjection {
	...
	public static SimpleProjection distance_pointToLine(final Point point, final String lineStringColumnName) {
		...
		public String toSqlString(Criteria criteria, int position, CriteriaQuery criteriaQuery) throws HibernateException {
			...
			String pointAsString = new StringBuffer("ST_GeomFromText( 'POINT(")
	...
	private static SimpleProjection distance_function(final Point point, final String locationColumnName, final String distanceFunction) {
		return new SimpleProjection() {
			...
			public String toSqlString(Criteria criteria, int position, CriteriaQuery criteriaQuery) throws HibernateException {
				...
				String sqlString = sb.append(distanceFunction).append("(").append(columnName).append(", ST_GeomFromText( 'POINT(").append(point.getX()).append(" ").append(point.getY()).append(")',").append(SRID.WGS84_SRID.getSRID()).append(")) as y").append(position).append("_").toString();
				return sqlString;
			}
